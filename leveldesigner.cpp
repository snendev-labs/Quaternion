...
void LevelDesigner::step() {
    ...
    if(!paused){
        ...
        if(timeSinceLastGoodguyFire>1.0/goodguyFireRate && readyFire == 1) {
            Bullet * temp;
        ...
        for(int k=0;k<goodguybullets->size();k++) {
            if((*goodguybullets)[k]->type == 0  || (*goodguybullets)[k]->type == 4){
                ...
                (*goodguybullets)[k]->circlemove(deltatime, goodguy->x(), goodguy->y(), goodguy->stepsize, goodguy->getDirection());
