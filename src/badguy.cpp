...
vector<double> Badguy::fireforward() {
    double ret[3] = {shipx, shipy, direction};
    vector<double> toret(ret, ret+3);
    return toret;
}
vector<double> Badguy::firedirect(int x, int y) {
    double ret[5]={shipx,shipy,atan2(y-shipy,x-shipx)};
    vector<double> toret(ret, ret+5);
    return toret;
}
vector<vector<double> > Badguy::firecircle(int x) {
    vector<vector<double> > toret;
    for(int k=0;k<x;k++) {
        double tmp[3]={shipx,shipy,2.0*k*M_PI/x};
        vector<double> temp(tmp, tmp+3);
...
vector<vector<double> > Badguy::firecircle(int x, double d) {
    vector<vector<double> > toret;
    for(int k=0;k<x;k++) {
        double tmp[3]={shipx,shipy,k*M_2_PI/x+d};
        vector<double> temp(tmp, tmp+3);
        toret.push_back(temp);
    }
    return toret;
